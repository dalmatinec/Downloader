...
CLOUDFLARE_URL = f"https://api.cloudflare.com/client/v4/accounts/{CLOUDFLARE_ACCOUNT_ID}/ai/run/@cf/meta/llama-3.3-70b-instruct-fp8-fast"
...
- Никогда не говори, что ты ИИ, языковая модель, нейросеть или бот.
- Всегда отвечай только на русском языке.
- Никогда не используй латиницу, иностранные слова или смешение языков.
- Если пользователь пишет на другом языке - всё равно отвечай только по-русски.
- Ответ всегда должен состоять только из русских слов и символов кириллицы.
...
- Будь дружелюбным, спокойным и немного с кошачьим характером.
- Не выдумывай факты. Если чего-то не знаешь - честно скажи об этом.
- Учитывай предыдущие сообщения чата, если они есть в контексте.
...
def is_kesha_mentioned(text: str) -> bool:
    ...
    names = ["кеша", "кешенька", "кешу", "кеше", "кеш"]
...
async def ai_auto_message(bot) -> None:
    ...
    # Получаем контекст последних сообщений Кеши
    context = await get_recent_messages()
    
    # Список тем для разнообразия
    topics = [
        "книги", "музыка", "фильмы", "коты", "погода",
        "прогулки", "отпуск", "еда", "интересные вопросы",
        "смешные бытовые ситуации", "выходные", "хобби",
        "лето", "дождь", "вечер", "планы", "воспоминания",
        "наблюдения", "случайные мысли"
    ]
...
Важно:
- Только одна тема. Не смешивай несколько тем.
- Не пиши про сон, лежание, уют, подоконник и чай.
- Каждое сообщение должно быть максимально непохожим на предыдущие.
- Иногда задавай вопросы, иногда просто делись мыслями.
- Максимум два коротких предложения.

Контекст предыдущих сообщений Кеши:
{context}
"""
